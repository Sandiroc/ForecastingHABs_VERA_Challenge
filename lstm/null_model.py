...
def batch_eval(X_test, y_test, model: Sequential, batches=(2, 4, 8, 16, 32)):
    """Find the optimal number of batches"""
...
def fit_model(model, X_train, y_train, epochs=30, batches=16):
    """ Wrapper function """
...
def seq_eval(X_test, y_test, model: Sequential, sequences=range(1,101)):
    # Find the optimal number of sequences 
    # inconclusive, number of sequences is variable.
    # standard sequence length is within +5 range of forecast window
...
def forecast(reservoir: str, forecast_horizon):
    # get fcre data
    data, reservoir = get_data(reservoir=reservoir)
    ...
    sequence_length = forecast_horizon + 1
    ...
    #print("Predicted chlorophyll-a value for " + 
    #    str(forecast_horizon) + " days into the future:", y_pred_inverse[-1])
    ...
    curr_forecast = y_pred_inverse[-1]
    #print(curr_forecast)
